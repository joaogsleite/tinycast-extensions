...
class KeePassLoader {
  ...
  private static parseCsvEntries = (entries: string): string[][] => {
    // parseCsv is a dependency-free parser: csv-parse relies on Node's
    // Buffer API which is incomplete in the Tinycast runtime.
    let entriesArray = parseCsv(entries, true)
      .sort((a: string[], b: string[]) => {
        ...
        const userA = a[2] ?? "";
        const userB = b[2] ?? "";
        return userA ? (userB ? userA.localeCompare(userB) : -1) : 1;
      })
      .filter((entry: string[]) => {
